...
!!! info "Understanding LLMs"
    Let's take the words one at a time *Large*, *Language*, and *Model*. Fundamentally, understanding LLMs means understanding *language modeling*, which boils down to algorithms which predict $P(word, character, token | previous text)$. Recall, neural networks are considered universal function approximators, where neural networks can get arbitrarily close to approximating _any_ function as the number of neurons approach $\infty$. Hence, why _large_ language models (based on neural networks), usually perform better than traditional language models. I highly recommend checking out this fantastic visualiztion of how LLMs work under the hood: [https://bbycroft.net/llm](https://bbycroft.net/llm) and these lecture notes: [https://mjt.cs.illinois.edu/dlt/](https://mjt.cs.illinois.edu/dlt/).
